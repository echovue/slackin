...
export default function slackin ({
  ...
  if (!linkedin) throw new Error('Must provide a `linkedin`.')
  ...
  app.get('/', (req, res) => {
    ...
    let page = dom('html',
      ...
      splash({ coc, path, css, name, org, logo, channels, active, total, gcaptcha_sitekey, linkedin})
    )
    res.type('html')
  ...
  app.post('/invite', json(), (req, res, next) => {
    ...
    if(!linkedin) {
        return res
        .status(400)
        .json({ msg: 'No Linkedin profile provided' + linkedin + '.'});
    }
